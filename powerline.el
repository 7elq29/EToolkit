...
(defun string-add (str floater)
  "Add string which represent an int."
   (+ floater
      (string-to-int str)))

(defun color-add (color floater)
  "Modify GDB color"
  (let ((s-list (string-to-list color)))
    (cond
     ((equal (substring color 0 1) "#")
      (setq c1 (string-add (substring color 1 3) -6))
      (setq c2 (string-add (substring color 3 5) -6))
      (setq c3 (string-add (substring color 5 7) -6))
      (format "#%02d%02d%02d" c1 c2 c3))
     ('t
      (setq c (string-add (substring color -2 nil) -6))
      (format "%s%02d"
       (substring color 0 -2)
       c)))))
...
\"..          \",
\".           \"};"
           (if color1 (color-add color1 6) "None")
           (if color2 (color-add color2 -6) "None"))
   'xpm t :ascent 'center))
